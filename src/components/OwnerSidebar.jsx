...
// SidebarLinkGroup component (previously in a separate file)
...
const OwnerSidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const isSmallDevice = useRef(window.innerWidth < 1024); // lg breakpoint is 1024px
  ...
  // Handle window resize to update isSmallDevice
  useEffect(() => {
    const handleResize = () => {
      isSmallDevice.current = window.innerWidth < 1024;
      // If screen size becomes large, ensure body scroll is enabled
      if (!isSmallDevice.current && sidebarOpen) {
  ...
  // Handle scroll locking when sidebar opens/closes
  useEffect(() => {
    if (isSmallDevice.current) {
      if (sidebarOpen) {
        // Lock scrolling when sidebar is open on small devices
        document.body.style.overflow = 'hidden';
      } else {
